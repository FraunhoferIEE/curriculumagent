...
class MyAgent(BaseAgent):
    def __init__(self, action_space, this_directory_path='./', savestate_path='./'):
        ...
        """ Define state variable names for saving the agent state"""
        varnames_base = ["space_prng", "seed_used"]
        varnames_baseAction = ["space_prng", "seed_used", "n", "global_vars",
                               "shape", "dtype", "attr_list_vect",
                               "_to_extract_vect"]
        varnames_custom = ["actions62", "actions146", "actions", "actions1255",
                           "chosen", "ppo", "last_step", "recovery_stack",
                           "overflow_steps"]
        self.state_variables_base = dict(
            zip(varnames_base, [None]*len(varnames_base)))
        self.state_variables_baseAction = dict(
            zip(varnames_baseAction, [None]*len(varnames_baseAction)))
        self.state_variables_custom = dict(
            zip(varnames_custom, [None]*len(varnames_custom)))
    ...
    def act(self, observation, reward, done):
        ...
        # case: secure
        threshold_this_step = 0.85  #0.999
        ...
        if (min_rho < 0.84) or (self.overflow_steps < 2):
    ...
    def save_state(self):
        # Write relevant agent state variables to files
        ...
        for key in self.state_variables_baseAction:
            if key != "space_prng":
                self.state_variables_baseAction[key] = getattr(
        ...
        for key in self.state_variables_custom:
            if key != "ppo":
                ...
                self.state_variables_custom[key] = 'Done'

        # Check if save directory exists, if not, make it!
        if not os.path.exists(self.savestate_path):
            os.makedirs(self.savestate_path)

        # Save all dicts and ppo
        ...
        self.ppo.save_weights(os.path.join(self.savestate_path, 'ppo_weights'))

    def load_state(self):
        # Load relevant agent state variables
        state_variables_base_load = np.load(os.path.join(
            self.savestate_path, 'state_variables_base.npy'),
            allow_pickle='TRUE').item()
        state_variables_baseAction_load = np.load(os.path.join(
            self.savestate_path, 'state_variables_baseAction.npy'),
            allow_pickle='TRUE').item()
        ...
        for key in state_variables_baseAction_load:
            if key != "space_prng":
                setattr(self.action_space, key,
                        state_variables_baseAction_load[key])
            else:
                self.action_space.space_prng.set_state(
                    state_variables_baseAction_load[key])

        # Update all custom state variables from dict and update PPO
        for key in state_variables_custom_load:
            if key != "ppo":
                setattr(self, key, state_variables_custom_load[key])
            else:
                self.ppo.load_weights(os.path.join(
                    self.savestate_path, 'ppo_weights'))


def make_agent(env, this_directory_path, savestate_path):
    my_agent = MyAgent(env.action_space, this_directory_path, savestate_path)
    return my_agent
