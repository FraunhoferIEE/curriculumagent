...
HOME_PATH = "" # Where can we find the 2022 dir and the models
...
def create_agents_and_env(seed=None):
    ...
    agent_original_85 = MyAgent(
        action_space=env.action_space,
        action_space_file=ppath / "2022" / "actions" / "actions.npy",
        best_action_threshold=0.85,
        **agent_kwargs)
    ...
    agents = {"Topo_Agent_95_2": topo_agent2,
              ...
              "Senior_original_85": agent_original_85,  
              }
...
def main(*, seed: int):
    ...
    res, surv_time = run_evaluation_of_env(agent_dict=agents,
                                           env=env,
                                           out_path=Path(DATA_PATH) / "seed_hugo_parallel" / str(seed),
                                           seed=seed,
                                           nb_process=20)
