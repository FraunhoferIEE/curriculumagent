...
The *Teacher* agent is the first agent in the pipeline of the project.
...
randomly sampling other unitary actions of different substations.

N-1 search
^^^^^^^^^^

In order to collect actions that make the powernet more robust to future overloads a teacher was developed that tries to
enforce `n+1 redundany(mostly called n-1 in german) <https://en.wikipedia.org/wiki/N%2B1_redundancy#Applications>`_.
